...
const votePollService = async (
  ...
  try {
    ...
    const newPoll: Poll = JSON.parse(JSON.stringify(poll));
    ...
    const index = newPoll.categories.findIndex(
      (category) => category.title === voteCategory
    );
...
export default votePollService;
